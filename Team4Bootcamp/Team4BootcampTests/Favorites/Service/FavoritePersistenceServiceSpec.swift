...
class FavoritePersistenceServiceSpec: QuickSpec {
    ...
    override func spec() {
        ...
        func flushData() {
            ...
            let objs = try! mockPersistantContainer.viewContext.fetch(fetchRequest) //swiftlint:disable:this force_try
        ...
        describe("FavoritePersistenceService") {
            context("Fetching Movies") {
                ...
                sut?.fetchMovies(context: mockPersistantContainer.viewContext) { result in
                    switch result {
